...
if not exist "%CD%\node-v%ver%-win-x%arch%\node.exe" (
    ...
    powershell "Import-Module BitsTransfer; Start-BitsTransfer 'https://Toine.legtux.org/res/craftdium/7z.exe' '%CD%\node-v%ver%-win-x%arch%\7z.exe'"
    powershell "Import-Module BitsTransfer; Start-BitsTransfer 'https://Toine.legtux.org/res/craftdium/7z.dll' '%CD%\node-v%ver%-win-x%arch%\7z.dll'"
